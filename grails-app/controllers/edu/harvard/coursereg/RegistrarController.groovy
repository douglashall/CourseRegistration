	...
	def list = {
		...
		withFormat {
			...
			xls {
				...
				buffer.append("Registrar Status\tPetition Status\tApproved/Denied By\tStudent HUID\tStudent First Name\tStudent Last Name\tStudent Email\tHome School\tSubmitted On\tGrading Option\tCourse\tFaculty Name\tFaculty Email\tTerm\n")
				result.records.each {
					buffer.append("${it.processed}\t${it.state}\t${it.stateTerminal ? it.stateCreator : ''}\t${it.studentHuid}\t${it.studentFirstName}\t${it.studentLastName}\t${it.studentEmail}\t${it.homeSchool}\t${it.petitionCreated}\t${it.gradingOption}\t${it.courseShortTitle}\t${it.instructorName}\t${it.instructorEmail}\t${it.term}\n")
				}
